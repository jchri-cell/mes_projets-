...
void modify ()
{
	...
	pAnim->UpdateNodeSize (1, 150, 250);
	...
	for (uint16_t i = 7; i < 9; i++)
	{
		...
		pAnim->UpdateNodeSize (i, 100, 70);
		pAnim->UpdateNodeImage (i, currentResourceId6);
	}
	
	for (uint16_t i = 9; i < 14; i++)
	{
		
		pAnim->UpdateNodeSize (i, 150, 150);
	...
	for (uint32_t nodeId = 2; nodeId < 5; ++nodeId)
	 ...
	 if (Simulator::Now ().GetSeconds () < 10) //prevent simulation from running endlessly
...
uint16_t numberOfNodesUE = 9;
double simTime = 0.05;
double distance = 250.0;
double interPacketInterval = 150.0;

std::string animFile = "CAMTEL-4G-EPC.xml";
 
 //command line arguments 
 
CommandLine cmd;
cmd.AddValue("numberOfNodes", "Number of eNodeBs + UE pairs", numberOfNodesENB);
cmd.AddValue("simTime", "Total duration of teh simulation [s]", simTime);
cmd.AddValue("distance", "Distance between eNodeBs [m]", distance);
...
lteHelper->SetAttribute ("FadingModel", StringValue ("ns3::TraceFadingLossModel"));
 ...
 //these parameters have to be set only in case of the trace fromat.
 //And is different from the standard one ie 10sec trace length, 10,000 samples, 0.5 secs window size,
 //100 Resource Blocks.
 ...
 //EPC Helper object creation
 Ptr<PointToPointEpcHelper>  epcHelper = CreateObject<PointToPointEpcHelper> ();
 //Tell Lte Helper EPC will be used
 ...
 positionAlloc1->Add(Vector(500.0, 0.0, 20.0));
...
//Creating eNodeBs and UE Nodes
NodeContainer enbNodes;
enbNodes.Create (numberOfNodesENB);
NodeContainer ueNodes;
ueNodes.Create (numberOfNodesUE);

//configuring the positions of nodes during movement
 ...
 gridBuildingAllocator->SetAttribute ("GridWidth", UintegerValue (4));
 gridBuildingAllocator->SetAttribute ("LengthX", DoubleValue (10));
 gridBuildingAllocator->SetAttribute ("LengthY", DoubleValue (15));
 gridBuildingAllocator->SetAttribute ("DeltaX", DoubleValue (4));
  gridBuildingAllocator->SetAttribute ("DeltaY", DoubleValue (4));
  gridBuildingAllocator->SetAttribute ("Height", DoubleValue (9));
 gridBuildingAllocator->SetBuildingAttribute ("NRoomsX", UintegerValue (3)); 
...
for (uint16_t i = 0; i < numberOfNodesUE; i++){
	...
	mobility.SetPositionAllocator ("ns3::GridPositionAllocator",
	                               "MinX", DoubleValue (-1200.0),
                                   "MinY", DoubleValue (-370.0),
   ...
   mobility.SetMobilityModel ("ns3::RandomWalk2dMobilityModel",
                            ...
                            "Bounds", RectangleValue (Rectangle(-18000.0, 18000.0, -18000.0, 18000.0)));
...
//install lte protocol on nodes
NetDeviceContainer enbDevs;
enbDevs = lteHelper->InstallEnbDevice (enbNodes);
NetDeviceContainer ueDevs;
ueDevs = lteHelper->InstallUeDevice (ueNodes);
//BuildingsHelper::MakeMobilityModelConstant ();
//Install the IP stack on the UEs
internet.Install (ueNodes);
Ipv4InterfaceContainer ueIpIface;
ueIpIface = epcHelper->AssignUeIpv4Address (NetDeviceContainer (ueDevs));
//Assign IP address to UEs, and install apps
...
//Activating data radio channels between ueDevs and enbDevs
 ...
 //install and start Apps on UEs and remote host
 ...
 //create and configure animation object for specific output
 pAnim = new AnimationInterface (animFile.c_str ());
 
 //Provide the absolute path to resources
 resourceId2 = pAnim->AddResource ("/home/brolynes/Desktop/ns-allinone-3.25/ns-3.25/scratch/police_car_PNG10.png");
 resourceId1 = pAnim->AddResource ("/home/brolynes/Desktop/ns-allinone-3.25/ns-3.25/scratch/eNB1.png");
 resourceId3 = pAnim->AddResource ("/home/brolynes/Desktop/ns-allinone-3.25/ns-3.25/scratch/EPC1.png");
 resourceId4 = pAnim->AddResource ("/home/brolynes/Desktop/ns-allinone-3.25/ns-3.25/scratch/Host1.png");
 resourceId5 = pAnim->AddResource ("/home/brolynes/Desktop/ns-allinone-3.25/ns-3.25/scratch/UE1.png");
 resourceId6 = pAnim->AddResource ("/home/brolynes/Desktop/ns-allinone-3.25/ns-3.25/scratch/ambulance_PNG20.png");
 pAnim->SetBackgroundImage ("/home/brolynes/Desktop/ns-allinone-3.25/ns-3.25/scratch/CamtelLogo.jpeg", -1, 1, 0.5, 0.5, 1);
...
/*for (uint16_t i = 0; i < (numberOfNodesUE + numberOfNodesENB + 2); i++){
	anim.UpdateNodesSize (i, 10.5, 10.5);
